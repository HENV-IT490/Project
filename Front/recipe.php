...
<?php
  ...
  if($type=='getName'){
    ...
    $request=file_get_contents("https://$host/Front/apiClient.php?".$getdata,false, stream_context_create($arrContextOptions));
  ...
  for($i=0;$i<count($analyzedResult['steps']);$i+=1)
  {
    ...
    for($j=0;$j<count($analyzedResult['steps'][$i]['ingredients']);$j+=1)
    {
     ...
     $url="https://$host/Front/apiClient.php?" ;
...
$jsonSimilar = file_get_contents("https://$host/Front/apiClient.php?type=getSimilar&recipeID=$recipeID", false, stream_context_create($arrContextOptions));
...
for($i = 0; $i < 3; $i+=1)
{
  ...
  echo "<br>";
  echo "<a href='https://$host/Front/recipe.php?type=getName&recipeID=$id'>{$similarArray[$i]['title']}</a>";
}


echo"<br><a href='https://$host/Front/customRecipe.php?recipe=$recipe'>Click to view/submit custom recipes</a>";
echo"<div id='respond'>
...
$.post('https://$host/Front/dbClient.php', { username: username, favoriteID: favID, favoriteName: favName, submit: 'favorites'},function(data){
...
$('#submit').click(function(){
  ...
  console.log(recipe);
  console.log(comment);
  $.post('https://$host/Front/dbClient.php', {username: username, comment: comment, submit: 'comment', recipe: recipe}, function(data){
  if(data != true){
    alert(data);
  }
  else{
...
echo"<script>
  ...
  $('#getComment').click(function(){
    $('#showComment').load('https://$host/Front/loadComment.php',{recipe: recipe},function(){
